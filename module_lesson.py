...
# ⇒単に「importしてるモジュール名を文字列で指定すればいい」とも思えるものの、処理の仕様上、中身が動的である場合には必要
...
その場合、Cさんが両方のモジュールをインポートしてから、a.my_function | b.my_function　と記述することで区別することができる。
 ...
 ③
 Python はコンパイル済みの各モジュールを __pycache__ ディレクトリの [モジュール名].[バージョン情報].pyc ファイルとしてキャッシュします。
 ...
 ④標準モジュールとしてsys がインポートされている
  sysにPYTHONPASS(環境変数)が含まれており、sys.path.append('/ufs/guido/lib/python') のように記述することで、環境変数を追加することができる

  ⑤dir(モジュール名) と記述することで、そのモジュール内でどのような名前が使われているか一覧を表示してくれる
    ⇒ただし、変数、モジュール、関数など モジュール内で定義された全ての名前が表示されるため、注意が必要とのこと
